...
#  Объявляем функцию get_matrix и пишем в ней параметры n, m и value.

def get_matrix(n, m, value):
    matrix = []

    # Создаём первый (внешний) цикл for для количества строк матрицы, n повторов.

    for i in range(n):

        # В первом цикле добавляется пустой список в список matrix.

        matrix.append([])

        # Создаём второй (внутренний) цикл for для количества столбцов матрицы, m повторов.

        for j in range(m):

            # Во втором цикле заполняется ранее добавленный пустой список значениями value.

            matrix[i].append(value)

    # После всех циклов возвращается значение переменной matrix.
